...
pub fn toProto(self: *Self, allocator: Allocator) !protocol.ByName(.AreaMapData) {
    ...
    while (streets.next()) |entry| {
        ...
        try protocol.addToList(allocator, &data, .street, protocol.makeProto(.AreaStreetInfo, .{
            .area_id = street.id,
    ...
    while (groups.next()) |entry| {
        ...
        try protocol.addToList(allocator, &data, .group, protocol.makeProto(.AreaGroupInfo, .{
