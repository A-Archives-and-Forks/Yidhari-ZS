...
pub fn toProto(self: *const Self, allocator: Allocator) !ByName(.DungeonInfo) {
    ...
    for (self.avatar_units.items) |unit| {
        try protocol.addToList(allocator, &proto, .avatar_list, try unit.toProto(allocator));
    }

    for (self.buddy_units.items) |unit| {
        try protocol.addToList(allocator, &proto, .buddy_list, try unit.toProto(allocator));
    }
