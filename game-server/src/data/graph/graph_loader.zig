...
fn loadMainCityGraph(map: *EventGraphTemplateMap, gpa: Allocator) !void {
    const content = try std.fs.cwd().readFileAllocOptions(gpa, main_city_config_path, 1024 * 1024, null, .of(u8), 0);
...
fn loadAllNpcGraphs(map: *EventGraphTemplateMap, gpa: Allocator) !void {
    ...
    while (true) {
        ...
        if (entry.kind == .file) {
            const content = try graph_dir.readFileAllocOptions(gpa, entry.path, 1024 * 1024, null, .of(u8), 0);
            defer gpa.free(content);
